...
how we acoomplished it on our Arch Linux desktop.

1. Enable forwarding of packets on the device
2. Enable the two interfaces
3. Add static routes for the two interfaces used by TRex. Previously, the PGEN has been 
   configured to use `11.11.11.2` as its IP address on one interface, and expects the DUT 
...
script, since many of them will have to be re-ran after every reboot. The script could look like 
this:
...
# Add static IP addresses to the devices. Replace the interface names if needed
ip a add 11.11.11.1/24 dev enp1s0f0
ip a add 12.12.12.1/24 dev enp1s0f1

# Add static routes for the traffic from TRex. These commands should work as-is
...
1. Configure the DUT to forward packets between the two interfaces, and set any parameters 
   you want to vary on the DUT.
2. Using TRex to generate traffic, measure forwarding latency of each packet, and export
   to a data blob.
...
```bash
# Print a summary of the data blob, and save a plot to plot.png. Cut away the first and last 
# second of the data
data-postprocessor timestamps-[date]-p0 validate --summary-only -c 1 -o plot.png

# Plot all packets in the data blob
data-postprocessor timestamps-[date]-p0 plot -p latency -c 1 -o plot.png
```
...
To disable X11, run the following commands:
```bash
...
4. Prefix all TRex commands with `taskset -c 0-3`. For example, to run the test mentioned earlier: 
   ```
   taskset -c 0-3 sudo ./_t-rex-64 --cfg path/to/config.yaml --lo -l 2000000 -f cap2/dns.yaml -d 60
   ```
